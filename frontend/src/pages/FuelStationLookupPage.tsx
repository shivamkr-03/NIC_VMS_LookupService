...
export const FuelStationLookupPage = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    if (!selectedEvent || !selectedDistrict || !selectedStation || !mobileNumber.trim()) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      {/* Title */}
      <div>
        <h1 style={{ fontSize: '2rem' }}>Fuel Station Lookup</h1>
        <p style={{ color: 'var(--text-muted)' }}>Audit fuel station accounts, coupon statistics, fuel categories, and bank payments.</p>
      </div>
      ...
      {result && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '1.25rem', marginBottom: '1.5rem' }}>
            <div>
              <span className={`badge ${result.stationInformation.active === 'Y' ? 'badge-success' : 'badge-danger'}`}>
                {result.stationInformation.active === 'Y' ? 'Active' : 'Inactive'}
              </span>
              <h2 style={{ fontSize: '1.75rem', marginBottom: 0 }}>{result.stationInformation.name}</h2>
            ...
            <div style={{ textAlign: 'right' }}>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                Oil Franchise: <strong style={{ color: '#fff' }}>{result.stationInformation.companyName || 'N/A'}</strong>
              </p>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
